...
const BookingJobs = (props) => {
    ...
    const bookingPriceTable = (
        <div
            className={`${progress === 5 ? 'col-lg-7 booking-summary-info-container' : 'col-lg-4'} d-flex ${width < 1024 ? 'justify-content-center' : 'justify-content-end'} px-3 orderContainer`}>
            {
                (progress === 5 && width > 1024) && (
        ...
        <div className="container booking" style={{ marginTop: `${width >= 1024 ? '14vh' : '24px'}` }}>
            <Switch>
                <Route exact path={path}>
                    <div>
                        <div className={`row justify-content-between ${progress === 5 ? 'align-items-center booking-container-space-between' : ''}`}>
                            {(width > 1024 || (width <= 1024)) &&
                                <div
                                    className={`${progress === 5 ? 'col-lg-5' : 'col-lg-8 pb-5'} ${(progress === 5 && width > 1024) ? '' : width > 767 ? '' : 'px-4'} bookingContainer`}
                                    style={{
                                        marginBottom: progress !== 10 ? '109px' : '',
                                        paddingBottom: 0
                                    }}>
                                    <div className={`flex-column mb-4 ${width <= 1024 ? '' : 'mt-3'}`}
                                        style={{ display: ((width <= 1024 && progress === 9) || (progress === 5 && width > 1024)) ? 'none' : 'flex' }}>
